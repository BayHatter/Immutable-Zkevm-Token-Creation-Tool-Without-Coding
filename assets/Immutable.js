...
export default function Immutable({ provider, account, connectWallet }) {
  ...
  const MAIN_WEBSITE_API = 'https://appsiko.com/api/get-token-data?network=immutable';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.log(`Using user config: wallet=${userWallet}, fee=${userFee} IMX`);
      } catch (error) {
  ...
  const handleViewToken = () => {
    if (contractAddress) {
      const etherscanUrl = `https://explorer.immutable.com/token/${contractAddress}`;
  ...
  return (
    <div className="container mx-auto p-6 max-w-3xl bg-background">
      <h1 className="text-3xl font-bold font-sans text-black mb-6 text-center animate-fadeIn">
        Create Your Immutable Zkevm Token
      </h1>
      ...
      <div className="mt-6 p-6 bg-muted rounded-xl animate-fadeIn">
        <h2 className="text-2xl font-bold font-sans text-black mb-4 relative">
          How to Create Your Token on the Immutable Zkevm Blockchain
        ...
        <p className="text-black font-bold font-sans leading-relaxed mb-4">
          Follow These Simple Steps To Create Meme Coin At Immutable Zkevm Within 10 - 30 Seconds:
        </p>
        <ol className="list-decimal list-inside text-black font-sans font-bold space-y-2">
          ...
          <li>
            Switch Your MetaMask Network to Immutable Zkevm. If The Immutable Zkevm Network Isn’t Showing On Metamask, You Can Add It As Your Custom Network. You Can Visit This Link For Getting RPC, Chain Id etc. Details <a href="https://chainlist.org/chain/13371" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">https://chainlist.org/chain/13371</a>, After Getting These Values You Can Add It As Custom Network.
          </li>
